...
task("export", "Export all to deployments", async (_) => {
  const readdir = promisify(fs.readdir);
  const deploymentsDir = path.resolve(path.join(__dirname, "..", "deployments"));
  const deploymentsSummary = path.resolve(path.join(__dirname, "..", "constants", "deployments.json"));
  ...
  for (const network of networks) {
    ...
    for (const jsonFileName of deploymentJSONs) {
      ...
      const deployData = JSON.parse((await promisify(fs.readFile)(jsonPath)).toString());
      const deployName = jsonFileName.split(".json")[0];
      deployments[network][deployName] = deployData.address;
    }
  }

  await promisify(fs.writeFile)(deploymentsSummary, stringify(deployments, { space: 4 }) + "\n");
